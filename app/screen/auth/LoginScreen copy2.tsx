...
const LoginScreen = ({navigation, route}: AuthStackProps) => {
  ...
  const handleMessage = async (event: any) => {
    ...
    try {
      ...
      if (action === 'now_play') {
        TrackPlayerInitializer().then(async () => {
          await TrackPlayer.add({
            id: title, // 가상 트랙의 고유 ID
            url: 'null', // 실제 음악 파일이 없으므로 빈 문자열 사용
            title: title, // 가상 트랙의 제목
            artist: artist, // 가상 트랙의 아티스트
            duration: duration, // 가상 트랙의 재생 시간 (초)
            // 추가적인 메타데이터 필드를 설정할 수도 있습니다.
          });
